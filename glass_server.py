...
from SimConnectExpansion.Enum import *
...
# Endpoint for triggering the Working Title CJ4 events using the MobiFlight WASM
@app.route('/wt_cj4_event/<event_name>/trigger', methods=["POST"])
def trigger_wt_cj4_event(event_name):
    # Not defined in the AircraftEvents() class in EventList.py, so Event ID needs to be
    # passed directly to a new Event class
    # ds = request.get_json() if request.is_json else request.form
    event_name = 'MobiFlight.' + event_name

    ev = Event(event_name.encode(), sm)       # Create the event object
    ev()                                # Now call the event
    status = "Event triggered: " + event_name
